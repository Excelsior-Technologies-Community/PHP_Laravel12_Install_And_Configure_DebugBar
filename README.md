...
A clean, submission‑ready Laravel 12 project demonstrating **installation and configuration of Laravel Debugbar** for development.

---
## Introduction

This project demonstrates the **installation and configuration of Laravel Debugbar in a Laravel 12 application**. Laravel Debugbar is a powerful development tool that provides real-time insights into application performance, database queries, routes, views, memory usage, and logged messages directly in the browser.

The purpose of this project is to help learners and freshers understand **how debugging works in Laravel**, how to properly install third-party development packages, and how to follow best practices by keeping debugging tools limited to the development environment only.

This project follows an industry-style setup and is suitable for **academic submission, internship evaluation, and self-learning**.
...
- Created a fresh **Laravel 12** application using official Laravel commands
- Installed **Laravel Debugbar** as a development dependency using Composer
- Published and explored the Debugbar configuration file
- Enabled Debugbar safely via environment variables
- Verified Debugbar functionality through a test route
- Ensured common mistakes and errors were identified and fixed correctly

By completing this project, you will gain practical knowledge of:

- Laravel package installation workflow
- Environment-based configuration using `.env`
- Debugging Laravel applications efficiently
- Understanding request lifecycle and performance metrics

This project is intentionally kept simple so the focus remains on **learning and understanding Debugbar**, not application complexity.

---
## Project Objective

* Install **Laravel 12** using official commands
* Install **Laravel Debugbar** as a dev dependency
* Publish and understand Debugbar configuration
* Verify Debugbar is working correctly in browser
* Follow best practices for development vs production

This project is prepared for **academic / internship / company submission**.
...
```bash
composer create-project laravel/laravel PHP_Laravel12_Install_And_Configure_DebugBar
cd PHP_Laravel12_Install_And_Configure_DebugBar
```

- This command installs Laravel 12 with all default folders and dependencies.
...
- Installed only for development (`--dev`)

- Auto‑discovered by Laravel (no manual provider needed)

---

## Step 3: Publish Debugbar Config

```bash
php artisan vendor:publish --provider="Barryvdh\Debugbar\ServiceProvider" --tag=config
```

This creates automatcally:

```
config/debugbar.php 
```

This file controls **what Debugbar shows** (queries, routes, memory, etc.).
...
- Debugbar works **only when `APP_DEBUG=true`**
...
use Barryvdh\Debugbar\Facades\Debugbar; // Use Facades Debugbar
...
- Note: **Do NOT use `use Debugbar;`**

Laravel uses the facade directly as `\Debugbar`.
...
- Debugbar should appear at the bottom of the page
...
* Request lifecycle
* SQL queries & execution time
* Loaded views
* Memory usage
* Route info
* Logged messages

This helps developers **debug faster and optimize performance**.
...
X Never enable Debugbar in production
...
<img width="1900" height="1056" alt="Screenshot 2025-12-13 195547" src="https://github.com/user-attachments/assets/3efd984a-6c31-4d30-932c-dbbb5f666974" />
